...
describe("RecipeSettingsPanel", () => {
  ...
  it("replaces, rather than duplicates, an existing rule for the same normalized ingredient key", () => {
    const onSave = vi.fn();
    render(
      <RecipeSettingsPanel
        recipe={recipe}
    ...
    // Same ingredient, different casing and a different volume unit.
    fireEvent.change(screen.getByLabelText(enMessages.ingredientKey), {
      target: { value: "butter" },
    });
    fireEvent.change(screen.getByLabelText(enMessages.gramsPerUnit), {
    ...
    expect(meta.ingredientConversionOverrides).toHaveLength(1);
    expect(meta.ingredientConversionOverrides[0]).toMatchObject({
      ingredientKey: "butter",
      gramsPerVolumeUnit: 226,
    });
  });
  ...
  it("accepts a safe graph asset path when native image listing is empty", () => {
    const onSave = vi.fn();
    render(
      <RecipeSettingsPanel
        recipe={recipe}
    ...
    fireEvent.change(screen.getByLabelText(enMessages.cover), {
      target: { value: "__custom__" },
    });
    const path = screen.getByLabelText("Cover asset path");
    for (const value of [
    ...
    fireEvent.change(path, { target: { value: "assets/cookie.png" } });
    fireEvent.click(screen.getByRole("button", { name: enMessages.save }));
    expect(onSave).toHaveBeenCalledWith(
      expect.any(Object),
      "assets/cookie.png",
  ...
  it("confirms before discarding an unsaved category addition", () => {
