...
describe("CookingMode", () => {
  ...
  it("labels the exit button Close, not Cancel - there is nothing to cancel here", () => {
    render(cookingMode(recipe));
    expect(screen.getByRole("button", { name: enMessages.close })).toBeTruthy();
  });
  ...
  it("gives the step progress indicator an accessible label", () => {
    render(cookingMode(recipe));
    const progress = document.querySelector("progress");
    expect(progress?.getAttribute("aria-label")).toBe(enMessages.stepProgress);
    expect(progress?.getAttribute("aria-valuetext")).toContain("1 / 2");
  });
