...
describe("recipe number formatting", () => {
  ...
  it("never renders a real amount as a bare 0", () => {
    // Half a teaspoon displayed in cups. Reported from real use as
    // "0 cup", which reads as the ingredient not being in the recipe.
    expect(formatRecipeNumber(0.5 / 48)).toBe("0.01");
    // Small enough that two decimals alone would still round to zero.
    expect(formatRecipeNumber(0.00492892159375)).toBe("0.0049");
    expect(formatRecipeNumber(0.0000031)).toBe("0.0000031");
  });

  it("still snaps a near-whole value and keeps a real zero", () => {
    expect(formatRecipeNumber(2.998)).toBe("3");
    expect(formatRecipeNumber(1.004)).toBe("1");
    expect(formatRecipeNumber(0)).toBe("0");
  });
});
