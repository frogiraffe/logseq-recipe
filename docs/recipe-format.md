# Lockstack Recipe Authoring Format

This document is the human- and machine-readable authoring contract for Lockstack Recipe.
...
Write a recipe as ordinary, readable Logseq blocks. Lockstack Recipe attaches and maintains technical metadata itself.

Do not require a visible `#Recipe` tag. Do not manually invent hidden Lockstack Recipe properties. Do not encode ingredient amounts in opaque JSON or a custom markup language.

A recipe should remain useful when Lockstack Recipe is disabled.
...
External assistants should prefer concise human-readable metadata rather than internal Lockstack Recipe fields.

## Ingredient line grammar

Lockstack Recipe intentionally accepts normal recipe-style ingredient lines.
...
Lockstack Recipe is not a general natural-language-number parser. Prefer numeric quantities for uncommon or large written-out numbers.
...
Lockstack Recipe does not try to semantically decompose every adjective or preparation phrase.
...
Lockstack Recipe distinguishes the recipe's **source measurement interpretation** from the user's **display measurement preference**.
...
Lockstack Recipe ships a small local registry of sourced common-ingredient rules and supports explicit per-recipe user overrides. If no reliable rule exists, the plugin must report that the conversion is unavailable instead of guessing a density.

External assistants should not invent conversion rules unless the user explicitly supplies a reliable relation.

## Step annotations

Cooking steps remain ordinary sentences. Lockstack Recipe derives useful annotations from explicit text while preserving the sentence itself.
...
Lockstack Recipe must never fabricate a Celsius/Fahrenheit temperature for these expressions.

## Parser locales

Lockstack Recipe's parser data supports:
...
Recipe categories and Lockstack Recipe tags are free-form organizational metadata. They are not restricted to a built-in vocabulary and are not required to become visible Logseq graph tags/pages.
...
`Lockstack Recipe: Create Recipe` creates a readable recipe skeleton and attaches internal recipe metadata itself.

### Convert to Recipe

`Lockstack Recipe: Convert to Recipe` is designed for structured or semi-structured Logseq content such as:
...
When a user asks you to convert a recipe into Lockstack Recipe format:
...
10. Do not add `#Recipe` merely for Lockstack Recipe.
11. Do not manually create Lockstack Recipe hidden properties, schema versions, section-role properties, cover references, or parser state.
12. Tell the user to paste the readable blocks into Logseq and run `Lockstack Recipe: Convert to Recipe` if the content was not created through the plugin.
...
- Lockstack Recipe plugin property names;
