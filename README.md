...
Logseq Recipe turns a normal Logseq DB graph into a recipe manager.
...
Open **Settings → Marketplace** in Logseq, search for **Logseq Recipe**,
...
Run **`Logseq Recipe: Create Recipe`** from the command palette. It
...
just edit the page like any other Logseq content; Logseq Recipe picks up
the changes automatically.

Already have recipe content in Logseq (your own notes, or something pasted
in)? Select the root block and run **`Logseq Recipe: Convert to Recipe`**.
...
Logseq Recipe doesn't scrape websites — instead, use any AI assistant as a
...
Logseq Recipe targets Logseq **DB graphs only** — file graphs are not
